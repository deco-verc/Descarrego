...
export default function WeeklyReportPage() {
  ...
  const generatePDF = () => {
    ...
    doc.text("RELATÓRIO SEMANAL", 105, 15, { align: "center" });
    
    doc.setFontSize(12);
    doc.setTextColor(30, 41, 59);
    doc.text(`Área: ${area.name}`, 14, 25);
    doc.text(`Período: ${start} até ${end}`, 14, 32);

    const body = reportData.map(d => [
      format(parseISO(d.date), "dd/MM (EEEE)", { locale: ptBR }),
    ...
    body.push([
      { content: 'TOTAIS DA SEMANA', styles: { fontStyle: 'bold', fillColor: [241, 245, 249] } },
    ...
    autoTable(doc, {
      head: [["Data", "Bruto", "Comissão", "Prêmios", "Final"]],
      body: body,
      startY: 40,
      theme: 'grid',
    ...
    doc.save(`relatorio-semanal-${area.name.toLowerCase().replace(/\s+/g, '-')}-${start.replace(/\//g, '-')}.pdf`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="card p-6 flex flex-wrap items-end gap-6">
        ...
        <div className="flex-1 min-w-[200px]">
          <label className="input-label">Escolha uma data da semana</label>
          <input 
            type="date" 
            value={selectedDate} 
      ...
      {loading ? (
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
           <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="card p-6 bg-white shadow-xl">
                 <p className="text-xs font-bold text-slate-400 uppercase">Bruto Semana</p>
                 <p className="text-2xl font-black text-slate-800">{formatCurrencyBRL(totals.entries)}</p>
              </div>
              <div className="card p-6 bg-white shadow-xl">
                 <p className="text-xs font-bold text-slate-400 uppercase">Comissão Semana</p>
                 <p className="text-2xl font-black text-red-600">{formatCurrencyBRL(totals.commission)}</p>
              </div>
              <div className="card p-6 bg-white shadow-xl">
                 <p className="text-xs font-bold text-slate-400 uppercase">Prêmios Semana</p>
              ...
              <table className="w-full text-center">
                 ...
                 <tbody className="divide-y divide-slate-100">
                    ...
                    <tr className="bg-slate-50 font-black text-slate-800">
                       <td className="py-4 px-6 text-left uppercase">Totais</td>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-100">
          ...
          <p className="text-slate-400 font-medium">Nenhum registro encontrado para esta semana.</p>
        </div>
      )}
    </div>
  );
