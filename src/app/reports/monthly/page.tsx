...
export default function MonthlyReportPage() {
  ...
  const generatePDF = () => {
    ...
    doc.text(`Relatório Mensal – ${area.name.toUpperCase()}`, pageWidth / 2, 20, { align: "center" });
    
    doc.setFontSize(10);
    doc.text(`Competência: ${monthYear}`, pageWidth / 2, 28, { align: "center" });
    ...
    const monthSlug = format(date, "MM-yyyy");
    doc.save(`Descarrego-${area.name}-Mensal(${monthSlug}).pdf`);
  };

  const monthStr = format(parseISO(selectedDate), "MMMM 'de' yyyy", { locale: ptBR });
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col items-center justify-center text-center py-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight [text-shadow:_0_1px_2px_rgb(0_0_0_/_10%)]">
            Relatório Mensal – {areas.find(a => a.id === selectedAreaId)?.name || "..."}
          </h1>
        </div>
        <p className="text-slate-500 font-bold uppercase text-sm tracking-widest">Mês de {monthStr}</p>
      </div>
      ...
      {loading ? (
              ...
              <table className="w-full text-right border-collapse">
                 ...
                 <tbody className="divide-y divide-slate-50">
                    ...
                    <tr className="bg-slate-100/50 font-bold text-slate-800">
                       <td className="py-6 px-6 text-left uppercase text-xs">Totais do Mês</td>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-100">
          ...
          <p className="text-slate-400 font-bold">Nenhum registro encontrado para este mês.</p>
        </div>
      )}
    </div>
  );
