...
import { Area, DailyRecord } from "@/types";
import { format, parseISO, startOfMonth } from "date-fns";
import { ptBR } from "date-fns/locale";
import { formatCurrencyBRL } from "@/lib/currency";
import { Loader2, FileDown, AlertCircle, Warehouse } from "lucide-react";
...
export default function CustomRangeReportPage() {
  const [areas, setAreas] = useState<Area[]>([]);
  const [selectedAreaId, setSelectedAreaId] = useState<string>("");
  const [startDate, setStartDate] = useState<string>(format(startOfMonth(new Date()), "yyyy-MM-dd"));
  const [endDate, setEndDate] = useState<string>(format(new Date(), "yyyy-MM-dd"));
  const [reportData, setReportData] = useState<DailyRecord[]>([]);
  ...
  const fetchReport = useCallback(async () => {
    if (!selectedAreaId || !startDate || !endDate) return;
    ...
    if (error) {
      ...
      if (records.length > 0) {
        setTotals({
          entries: records.reduce((acc, curr) => acc + Number(curr.total_entries), 0),
          commission: records.reduce((acc, curr) => acc + Number(curr.total_commission), 0),
          prizes: records.reduce((acc, curr) => acc + Number(curr.total_prizes), 0),
          final: records.reduce((acc, curr) => acc + Number(curr.total_final), 0),
        });
      } else {
        setTotals(null);
      }
    ...
    setLoading(false);
  }, [selectedAreaId, startDate, endDate, supabase]);
  ...
  const generatePDF = () => {
    if (!reportData.length || !totals) return;
    ...
    const startStr = format(parseISO(startDate), "dd/MM/yyyy");
    const endStr = format(parseISO(endDate), "dd/MM/yyyy");
    ...
    doc.text(`Relatório por Período – ${area.name.toUpperCase()}`, pageWidth / 2, 20, { align: "center" });
    
    doc.setFontSize(10);
    doc.text(`Período: ${startStr} a ${endStr}`, pageWidth / 2, 28, { align: "center" });
    ...
    doc.save(`Descarrego-${area.name}-Periodo(${startStr.replace(/\//g, '.')}-a-${endStr.replace(/\//g, '.')}).pdf`);
  };
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col items-center justify-center text-center py-4">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight [text-shadow:_0_1.5px_3px_rgb(0_0_0_/_15%)]">
            Relatório por Período – {areas.find(a => a.id === selectedAreaId)?.name || "..."}
          </h1>
        </div>
        <p className="text-slate-500 font-bold">
          Período: {format(parseISO(startDate), "dd/MM/yy")} a {format(parseISO(endDate), "dd/MM/yy")}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-100">
          ...
          <p className="text-slate-400 font-bold">Nenhum registro encontrado para este período.</p>
        </div>
      )}
    </div>
  );
